...
| **[Raspberry Pi Pico SDK](soc.md)** | RP2040, RP2350 and RP2354. One command: `install_rpi_online.py`, or `install_rpi_offline.py` from a local copy. The SoC package supplies the vector names, the tick and the multi-core glue, so there is no PendSV or SysTick step to do by hand |
