...
| [`os_main_queue.c`](../examples/kernel/os_main_queue.c) | The same items through two queues - one `OS_QUEUE_DEFINE`, one heap-backed via `os_queue_init_dynamic` - to show that only the storage differs, never the calls | `QUEUE` (+ `ALLOC` for the dynamic half) | 1 |
