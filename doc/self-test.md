...
4. **Give the suite the `SVC` vector**, so the ISR-safe APIs are exercised from a
   ...
   **System Core → NVIC → Code generation** - otherwise CubeMX emits its own
   ...
   That is the whole step, and it is what every board in this project does.

   **Only** if your application genuinely uses `SVC` itself - a SoftDevice, TF-M,
   a vendor ROM API - keep its handler and share the vector instead: set
   `OS_CONFIG_TEST_SVC_VECTOR` to `0` and call `os_test_isr_entry()` from inside
   the handler you already have. On CubeMX that means a `USER CODE` block, so
   ...
   This is for a vector you cannot give up, not a second way to run the tests -
   prefer the checkbox whenever `SVC` is free. Forget the call and the suite says
   so with a SKIP naming it, never a hang.
...
in the
[kernel reference → Self-test suite](testing.md#self-test-suite).
