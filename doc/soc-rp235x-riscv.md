...
[by hand](pico-sdk.md#manual---step-by-step). Select the RISC-V platform when
...
The Pico VS Code extension and the CMake cache both express it the same way.

Everything below is what the package *is*, and what your application owes it.

## Layout

Unlike the Arm sibling this package does **not** compile in
...
| **Tick** | `mtime`/`mtimecmp`, taken as ordinary system IRQ 29 (`SIO_IRQ_MTIMECMP`) rather than on `mip.MTIP` - see [Notes](#notes). Installed as a *shared* handler, so an application handler already on that line is not displaced |
...
Setting `OS_CONFIG_CORE_COUNT` to 2 is the whole of enabling the second Hazard3.
...
**Cache coherency is not a concern here** - there is no data cache between the
cores and SRAM - so the second `OS_CONFIG_CORE_COUNT` precondition in
`os_config.h` is satisfied for free, exactly as on the Arm side.
