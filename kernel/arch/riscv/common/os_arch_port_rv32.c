 ...
 * file is never added to a build directly. See kernel/arch/riscv/hazard3/os_arch_port.c.
 *
...
/******************************************************************************************************/
/**
 * @brief Assert that the calling interrupt may use kernel APIs.
 *
 * Nothing to check on RISC-V. The ARM port uses this to catch an interrupt above
 * OS_CONFIG_MAX_SYSCALL_IRQ_PRIORITY calling into the kernel while BASEPRI leaves it running; here
 * a kernel critical section clears mstatus.MIE and masks everything, so no such interrupt exists.
 */
