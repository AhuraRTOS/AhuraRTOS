 ...
 * not a qualifier. Every access happens between this module's own mask raise and its matching
 * restore, each core indexes its own slot, and the port's mask sequences carry a "memory"
 * clobber so the compiler cannot move an access out of the masked region either.
 *
 * volatile would add no safety - it does not make count++ indivisible - and would forbid
 * keeping the value in a register, costing os_critical_exit() four reloads of the same word
 * on a path every mutex, semaphore, queue and event operation runs through.
 */
