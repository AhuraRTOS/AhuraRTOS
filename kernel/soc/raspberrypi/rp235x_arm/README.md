...
`_arm` because the same silicon can boot Hazard3 RISC-V cores instead. That
would be a sibling package, `raspberrypi/rp235x_riscv`, not a switch inside this
one - see `soc.cmake` for why.

## Layout

Most of this package is `../common/`, compiled in alongside the file here:

| | |
|---|---|
| `../common/soc_common.c` | Core id, kernel spinlock, `SystemCoreClock`, `isr_systick`, core 1 launch - identical on every RP2 chip, because it is all SIO or plain SDK |
...
TrustZone, no doorbells against doorbells.
...
has not run on hardware - see its README.
