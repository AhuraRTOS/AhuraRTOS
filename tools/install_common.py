...
"""
Shared machinery for the AhuraRTOS installers. Not runnable on its own.

Everything here is the part of an install that does not depend on which silicon
or which code generator produced the project: reading a text file so its byte
identity survives editing, the managed-block sentinels, computing a diff,
asking for confirmation, writing with rollback, and finding or downloading the
repository.

What stays in the per-target installers is only what genuinely differs - where
the anchors are and what the payload says:

    install_stm32_online.py / install_stm32_offline.py    CubeMX projects
    install_rpi_online.py   / install_rpi_offline.py      Pico SDK projects

Each pair is one implementation, not two: the offline half imports the online
half and replaces only how the repository is found. So a fix to the editing
logic lands in one place and every installer follows.

The split has one rule worth stating, because it is what keeps this file
honest: nothing here may know about CubeMX, USER CODE sections, the Pico SDK,
or any vendor. If a function needs that knowledge it belongs in the installer,
not here.
